...
export class NLPService {
  ...
  async analyzeEntities(text: string): Promise<EntityAnalysisResult[]> {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const analysisResult: EntityAnalysisResult[] = entities.map(entity => ({
          name: entity.name || '',
   ...
   */
  private extractCategoriesFromEntities(entities: EntityAnalysisResult[], text: string): string[] {
    const categories: string[] = [];
   ...
   */
  private fallbackEntityAnalysis(text: string): EntityAnalysisResult[] {
