...
const [AppContext, useAppStore] = createContextHook(() => {
  ...
  const openInMaps = useCallback((place: Suggestion | PlaceResult | AdvertisedPlace) => {
  ...
  return {
    ...
    getDiscoveryStats: useCallback(() => ({
      totalPlaces: state.currentResults?.places.length || 0,
      hasAdvertised: state.currentResults?.places.some(place => 'isAdvertised' in place && place.isAdvertised) || false,
