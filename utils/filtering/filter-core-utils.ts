 ...
 * 
 * Consolidated utility functions for all filter operations.
 * Merges functionality from filter-utils.ts and filter-utilities.ts
...
export class FilterValidation {
  ...
  static validateUnifiedFilters(filters: UnifiedFilters): FilterValidationResult {
    ...
    if (filters.userLocation) {
      if (!this.isValidCoordinates(filters.userLocation)) {
        errors.push('Invalid user location coordinates');
      }
    }

    // Set defaults for null values
    if (normalizedFilters.minResults === undefined) {
      normalizedFilters.minResults = 5;
    }
  ...
  /**
   * Validate filter combinations for compatibility
   */
  private static validateFilterCombinations(filters: UnifiedFilters, warnings: string[]): void {
    // Check category + mood compatibility
    if (filters.category === 'food' && filters.mood !== null && filters.mood > 80) {
      warnings.push('Very high mood (hype) with food category might limit results');
    }

    // Check social context + category compatibility  
    if (filters.socialContext === 'solo' && filters.category === 'activity') {
      warnings.push('Solo social context with activity category might limit results');
    }

    // Check budget + category compatibility
    if (filters.budget === 'PPP' && filters.category === 'activity') {
      warnings.push('Premium budget with activity category might limit results');
    }

    // Check time + category compatibility
    if (filters.timeOfDay === 'morning' && filters.category === 'activity') {
      warnings.push('Morning time with activity category might limit results');
    }
  }
  ...
  private static isValidTimeOfDay(timeOfDay: string): boolean {
    return ['morning', 'afternoon', 'night'].includes(timeOfDay);
  }
...
export class FilterConversion {
  static readonly CONVERTERS = {
    legacyToNew: {
      category: (category: string): LookingForOption => {
        if (category === 'something-new') return 'something_new';
      ...
      socialContext: (social: string | null | undefined): SocialContext | undefined => {
        if (social === 'with-bae') return 'withBae';
    ...
    newToLegacy: {
      category: (lookingFor: LookingForOption): string => {
        if (lookingFor === 'something_new') return 'something-new';
        return lookingFor;
      },
      ...
      socialContext: (social: SocialContext | undefined): string | null => {
        if (social === 'withBae') return 'with-bae';
        return social || null;
      },
  ...
  static convertLegacyFilters(filters: DiscoveryFilters): SearchParams {
    const searchParams: SearchParams = {
      ...
      lookingFor: this.CONVERTERS.legacyToNew.category(filters.category),
      mood: this.CONVERTERS.legacyToNew.mood(filters.mood),
