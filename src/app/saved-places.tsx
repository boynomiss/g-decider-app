...
import { PlaceMoodData } from '../features/discovery/types';
...
export default function SavedPlacesScreen() {
  ...
  const { savedPlaces, removePlace, clearAll, isLoading } = useSavedPlaces();

  const handleRemovePlace = useCallback((place: PlaceMoodData) => {
    Alert.alert(
      'Remove Place',
  ...
  const handleRemovePlaceForCard = useCallback((place: PlaceMoodData) => {
    handleRemovePlace(place);
  }, [handleRemovePlace]);

  const handleRemovePlaceForCardInline = useCallback((place: PlaceMoodData) => {
    handleRemovePlaceForCard(place);
  }, [handleRemovePlaceForCard]);
          ...
          savedPlaces.map((place, index) => {
            ...
            return (
              <EnhancedPlaceCard
                key={place.id || index}
                place={placeData}
                onSave={handleSavePlace} // Already saved
                isSaved={true}
                showRemoveButton={true}
                onRemove={() => handleRemovePlaceForCardInline(place)}
