...
import { googleNaturalLanguageClient } from '../../../../../services/api/api/google-api-clients';
...
export class EntityMoodAnalysisService implements IEntityMoodService {
  ...
  async analyzeFromReviews(reviews: ReviewEntity[], placeCategory: string): Promise<MoodAnalysisResult> {
    try {
      ...
      this.logError('entity-mood-analysis', 'Analysis failed');
      return this.getFallbackAnalysis(placeCategory);
    }
  }
   ...
   */
  async analyzeEntities(text: string): Promise<any[]> { // Changed return type to any[] as EntityAnalysisResult is removed
   ...
   */
  async extractMoodDescriptors(entities: any[], entityWeights?: Map<string, number>): Promise<string[]> { // Changed type to any[]
   ...
   */
  calculateConfidence(entities: any[], reviewCount: number): number { // Changed type to any[]
   ...
   */
  private async performEntityAnalysis(text: string): Promise<any[]> { // Changed return type to any[]
   ...
   */
  private async extractEntitiesFromReviews(reviews: ReviewEntity[]): Promise<any[]> { // Changed return type to any[]
    const allEntities: any[] = []; // Changed type to any[]
    ...
    for (const review of reviews) {
      try {
        ...
        this.logWarn('entity-extraction', 'Failed to analyze review');
        continue;
      }
    }
   ...
   * Calculate entity weights based on review quality
   */
  private calculateEntityWeights(entities: any[], reviews: ReviewEntity[]): Map<string, number> { // Changed type to any[]
  ...
  private calculateEntityBasedMoodScore(
    entities: any[], // Changed type to any[]
...
export const entityMoodUtils = {
   ...
   */
  validateEntityResult: (result: any): boolean => { // Changed type to any
    return !!(result.name && 
             result.type && 
