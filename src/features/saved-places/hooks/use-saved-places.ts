...
import { SaveablePlace, SavedPlace, UseSavedPlacesReturn } from '../types/saved-places-interfaces';
import { Suggestion } from '../../discovery/types';

// Storage key for saved places
...
export const useSavedPlaces = (): UseSavedPlacesReturn => {
  ...
  const removePlace = useCallback(async (placeId: string) => {
    try {
      ...
      await AsyncStorage.setItem(SAVED_PLACES_KEY, JSON.stringify(updated));
      setSavedPlaces(updated);
      console.log('🗑️ Removed place with ID:', placeId);
    } catch (error) {
